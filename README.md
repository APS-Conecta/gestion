...
`epidemiologia`, cloned from its own repository into `apps/` and declared in `OWN_APPS`
([ADR-0003](docs/adr/0003-this-stack-ships-a-custom-app.md), reversing AD-1) — `apps/` otherwise holds only
upstream apps unpacked from the vendored tarballs, and all of it is gitignored. The REM analyzer is the next
Layer-2 app, on the same terms. White-labeling ships as the **`themes/apsconecta/`
