...
2. **Create your local env file** (gitignored — never committed).
   ...
   *The stack boots with the placeholders, but don't leave real deployments on them.*
   `SITE` names the clinic under `sites/` that `make seed` will provision. `SITE=dev` is the
   synthetic example site the repo ships; for a real CESFAM, write your own first:
   ```bash
   scripts/deis.py cesfam la-florida        # find it in the DEIS register
   scripts/deis.py 114302 --new mi-cesfam   # writes sites/mi-cesfam/site.sh, then set SITE=mi-cesfam
   ```
   No real clinic is committed here ([`provisioning/README.md`](provisioning/README.md)).
