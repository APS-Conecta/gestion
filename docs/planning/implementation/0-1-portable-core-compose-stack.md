...
- [x] **Task 1: `compose.yaml` with the three core services** (AC: 1, 3)
  - [x] `nextcloud` service — image `nextcloud:33-apache`; publish `127.0.0.1:${HTTP_PORT}:80` (loopback only); `depends_on` postgres + redis with `condition: service_healthy`; named volume `nextcloud_data:/var/www/html`; `extra_hosts: ["host.docker.internal:host-gateway"]`; `curl` healthcheck on `/status.php`.
...
- All 4 ACs verified end-to-end against a live stack (initial verification on free host port 8091; `.env.example` now ships `HTTP_PORT=8180`). Stack **stopped** after verification to free shared-host resources — restart with `make up`.
