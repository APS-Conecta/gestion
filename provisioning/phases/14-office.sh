# Phase 14 — Euro-Office connector configuration.  OWNER: AD-5 + the installer suite (slice 16's AIO arm).
# Runs after 12-apps (which installs the `eurooffice` connector) and before 15-branding.
#
# This phase owns EVERYTHING the connector needs, including the trusted_domains repair that used
# to live in a make target (#81) — see the note on it below.
...
if [ "$aio" = 0 ]; then
  ...
  # a .env missing the key produced three different answers and wrote a silently-wrong browser
  # URL. .env.example ships it and compose.yaml requires it, so absence means a broken .env —
  # say so. (Under AIO the key is unused — the gate lives inside the compose branch.)
  ...
  # Unset = localhost, right for every posture this repo ships: compose.yaml publishes both ports
  # on 127.0.0.1. Set it when something proxies them outward, so the seed converges instead of
  # clobbering.
  ...
  # The other half of StorageUrl, and the one piece of this that is not appconfig. The document
  # server fetches documents FROM Nextcloud at the service name `nextcloud`, which Nextcloud
  # rejects with HTTP 400 unless it is a trusted domain.
  #
  # It lived in `make office-eurooffice` and was AD-2's one documented exception until #81 dropped
  # the eurooffice profile and retired it — docs/adr/0000-inherited-decisions.md §AD-2 owns that
  # history.
  #
  # Not through config_system_set: that helper compares a scalar, and this is an ARRAY where the
  # key to write is the next free INDEX. Query-before-set is done directly instead — `occ` prints
  # one domain per line, so a fixed-string whole-line match is the membership test and the line
  # count is the index. Reading through conf_get would return the whole array as JSON and still
  # need parsing.
...
# Only the ODF names: formatsSetting() overrides just the keys present, so OOXML keeps its
# defaults.
...
if [ "$aio" = 0 ]; then
  ...
  #
  # REQUIRED, not skipped-when-absent (#81). It used to log a skip, because a seed must not die
  # over an optional component. The component is not optional any more, and the skip branch had
  # become unreachable in any case: compose.yaml guards this key with ${OFFICE_JWT_SECRET:?}, so
  # an unset value stops the stack from starting long before a phase could run against it.
  # (Under AIO the entrypoint's own jwt rewrite owns this key — the gate lives in the compose
  # branch with the write.)
  ...
  log "AIO: eurooffice jwt_secret is the entrypoint's (its own document server's secret)"
fi
