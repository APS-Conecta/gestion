...
# is this one. A committed placeholder holds the path open — scripts/test.sh requires every url()
# in server.css to resolve on disk, and a fresh clone has never run this phase.
#
...
/* GENERATED by provisioning/phases/15-branding.sh — do not edit by hand.
   Rewritten on every \`make install\`; the committed copy is a placeholder that keeps
   scripts/test.sh green on a checkout that has never been seeded. */
:root {
