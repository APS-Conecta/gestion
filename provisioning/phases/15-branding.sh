...
# --- The clinic's name, into the theme (#84) ---
# The ONLY per-clinic byte in themes/. server.css @imports this file and reads `--aps-clinic` in
# the header slot and above the login heading; everything else in the theme is identical on every
# install, which is what keeps themes/ a single committed artifact across clinics.
#
# Written on the HOST, not through occ: themes/ is a bind mount, so the file the container serves
# is this one. The committed copy holds the reference clinic and the path open — scripts/test.sh
# requires every url() in server.css to resolve on disk, and a fresh clone has never run this phase.
#
...
if ! grep -qF -- "--aps-clinic: \"${_clinic_escaped}\";" "$SITE_CSS" 2>/dev/null; then
  cat > "$SITE_CSS" <<CSS
/* GENERATED by provisioning/phases/15-branding.sh — do not edit by hand.
   Rewritten on every \`make install\` that finds a different clinic; the committed copy holds the
   reference clinic, which keeps scripts/test.sh green on a checkout that has never been seeded. */
:root {
